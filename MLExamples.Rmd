...
trainsetkclus <- kmeans(unlabeled,nclusters,nstart = 20)
trainset$Kclus = trainsetkclus$cluster
trainset$pca1 = modpca$x[,1]
trainset$pca2 = modpca$x[,2]

plot_ly(as.data.frame(modpca$x), x=~PC1, y=~PC2, type = "scatter", color = as.factor(trainset$y))
...
```{r}
plot_ly(as.data.frame(modpca$x), x=~PC1, y=~PC2, type = "scatter", color = as.factor(trainset$Kclus))
```
...
happiness_raw <- read_csv('happiness/happiness.csv')
...
We can form some tangible conclusions from the countries that were placed in each of the clusters. Under cluster 2 we can find:
...
In order to avoid overfitting, cross-validation should be considered for the LDA and QDA methods, as well as any other predictive methods which involve classes.
